...
require_once UPDATE_PATH . '/src/Tools.php';
...
output(color_string("Shopware 4.1 Update", 'green'));
...
print_header("Category Migration");
rebuildAllAssignments($categoryAdmin, $tools);

print_header('Update "Similar Shown"-Index');
initSimiliarShown($tools);

print_header('Update "Top Seller"-Index');
initTopSeller($tools);

print_header('Update "Customers also bought"-Index');
initAlsoBought($tools);
...
function clearCache()
{
    ...
    if (is_dir(SW_PATH . '/engine/Shopware/Proxies')) {
        ...
        print_warn("Please remove missing directory: rm -rf " . SW_PATH . '/engine/Shopware/Proxies');
        $hasErrors = true;
    }

    if (is_dir(SW_PATH . '/engine/Shopware/Attribute')) {
        print_error("Could not remove directory" . SW_PATH . '/engine/Shopware/Attribute');
        print_warn("Please remove missing directory: rm -rf " . SW_PATH . '/engine/Shopware/Attribute');
        $hasErrors = true;
    }
...
function rebuildAllAssignments(Shopware\Components\Model\CategoryDenormalization $categoryAdmin, \Shopware\Update\Tools $tools)
{
    $categoryAdmin->removeAllAssignments();
    ...
    if ($count > 1000) {
        $batchSize = ceil($count / 100);
    } else {
        $batchSize = ceil($count / 10);
    }

    $adapter = new Zend_ProgressBar_Adapter_Console();
    $progressBar = new Zend_ProgressBar($adapter, 0, $count);
    for ($offset = 0; $offset < $count; $offset += $batchSize) {
        $categoryAdmin->rebuildAllAssignments($batchSize, $offset);
        $progressBar->next($batchSize);
    }
    $progressBar->finish();
}

function initSimiliarShown(\Shopware\Update\Tools $tools)
{
    ...
    $progressBar = new Zend_ProgressBar($adapter, 0, $count);
    for ($offset = 0; $offset < $count; $offset += $batchSize) {
        $tools->initSimilarShown($offset, $batchSize);
        $progressBar->next($batchSize);
    }
    $progressBar->finish();
}

function initAlsoBought(\Shopware\Update\Tools $tools)
{
    $count = $tools->getArticleCount();
    if ($count > 100) {
        $batchSize = ceil($count / 100);
    ...
    $progressBar = new Zend_ProgressBar($adapter, 0, $count);
    for ($offset = 0; $offset < $count; $offset += $batchSize) {
        $tools->initAlsoBought($offset, $batchSize);
        $progressBar->next($batchSize);
    }
    $progressBar->finish();
}

function initTopSeller(\Shopware\Update\Tools $tools)
{
    $count = $tools->getArticleCount();
    if ($count > 100) {
        $batchSize = ceil($count / 100);
