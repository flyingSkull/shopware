        ...
        <div class="middle">
                ...
                <div class="article_box">
                    ...
                    <div class="thumbnail">
                        <a href="{$sArticle.linkDetails}" title="{$sArticle.articleName}"
                           class="artbox_thumb" {if $sArticle.image.src}
                        style="background: url({$sArticle.image.src.1}) no-repeat center center"{/if}>
                            {if !$sArticle.image.src}<img src="{link file='frontend/_resources/images/no_picture.jpg'}"
                                                          alt="{s name='ListingBoxNoPicture'}{/s}" />{/if}
                        </a>
                    </div>
            ...
            <div class="actions">
                {block name='frontend_checkout_ajax_add_article_action_buttons'}
                    ...
                    <a href="{url action='cart'}" class="button-right large right"
                       title="{s name='AjaxAddLinkCart'}{/s}">
            ...
            {if $sCrossSimilarShown|@count || $sCrossBoughtToo|@count}
                ...
                <div class="slider_modal">
                    ...
                    {foreach from=$sCrossSellingArticles item=article}
                        ...
                        <div class="article_box">
                            {if $image}
                                <a style="background: url({$image}) no-repeat scroll center center transparent;"
                                   class="artbox_thumb" title="{$article.articleName|escape}"
                                   href="{$article.linkDetails}">
                                </a>
                            {else}
                                <a class="artbox_thumb no_picture" title="{$article.articleName|escape}"
                                   href="{$article.linkDetails}">
                                </a>
                            {/if}
                            <a title="{$article.articleName}" class="title"
                               href="{$article.linkDetails}">{$article.articleName|truncate:28}</a>
                            {if $article.purchaseunit}
                                <div class="article_price_unit">
                                    <p>
                                        <strong>{se name="SlideArticleInfoContent"}{/se}
                                            :</strong> {$article.purchaseunit} {$article.sUnit.description}
                                    </p>
                                    {if $article.purchaseunit != $article.referenceunit}
                                        <p>
                                            {if $article.referenceunit}
                                                <strong class="baseprice">{se name="SlideArticleInfoBaseprice"}{/se}
                                                    :</strong>
                            ...
                            <p class="price">
                                <span class="price">{if $article.priceStartingFrom && !$article.liveshoppingData}{s name='ListingBoxArticleStartsAt'}{/s} {/if}{$article.price|currency}
                                    *</span>
                            </p>
                        </div>
