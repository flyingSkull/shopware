...
{* Group description *}
    ...
    <select
        {if $groupID gt 0&&empty($sArticle.sConfigurator[$pregroupID].user_selected)}disabled="disabled"{/if}name="group[{$sConfigurator.groupID}]"
    ...
    {* Please select... *}
        ...
        {foreach from=$sConfigurator.values item=configValue name=option key=optionID}
            {if !isset($configValue.active)||$configValue.active==1}
                <option {if $configValue.selected&&$sConfigurator.user_selected}selected="selected"{/if}
