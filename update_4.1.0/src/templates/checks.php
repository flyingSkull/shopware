...
<div id="start">
    ...
    <form action="<?php echo $app->urlFor('checks', array()); ?>" method="get">
        ...
        <span class="help-block">
            <?php echo $language["step2_files_delete_info"];?>
        </span>
        ...
        <table class="table table-striped">
            ...
            <tbody>
            <?php foreach ($filesToDelete as $fileToDelete => $result): ?>
                <?php $class = ($result) ? 'success' : 'error'; ?>
                <tr class="<?php echo $class; ?>">
                    <td><?php echo $fileToDelete ?></td>
                    <td><?php echo $result == true ? '<i class="icon-ok-sign"></i>' : '<i class="icon-minus-sign"></i>' ?></td>
                </tr>
            <?php endforeach; ?>
            </tbody>
        </table>
